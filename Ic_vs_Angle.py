...
def load_and_plot_files(files, filter_angle=None, filter_field=None):
    ...
    for i, file in enumerate(files):
        try:
            ...
            if field_strength not in field_dict:
                field_dict[field_strength] = {"angles": [], "critical_currents": []}
            field_dict[field_strength]["angles"].append(angle)
            field_dict[field_strength]["critical_currents"].append(Ic)
    ...
    cmap = cm.get_cmap('YlOrRd', len(field_dict))  # Replace 'viridis' with 'abbott' if you have it available
    colors = [cmap(i / len(field_dict)) for i in range(len(field_dict))]

    zero_field_index = list(field_dict.keys()).index(0.15)  # Assuming 0 degrees exists in angle_dict
    colors[zero_field_index] = (1.0, 0.94, 0.67, 1.0)  # Custom yellow shade (RGBA format)
    #sorted_field_dict = dict(sorted(field_dict.items()))

    for i, (field_strength, data) in enumerate(field_dict.items()):
        angle = np.array(data["angles"])
        critical_currents = np.array(data["critical_currents"])

        # Sort the data by angle
        sorted_indices = np.argsort(angle)
        angle = angle[sorted_indices]
        ...
        plt.plot(
            angle,
            critical_currents,
            label=f"{field_strength} T",
            marker="o",
            linestyle="--", 
            color=colors[i], linewidth=2
        )
    plt.xlabel("Angle of Applied Field [°]", fontsize = 14)
    plt.ylabel("$I_{c}$ [A]", fontsize = 14)
    plt.legend(title="Magnetic Field", fontsize=14, title_fontsize=14)
